		...
		private void UpdateActorCharacter(IActorCharacterProvider provider, bool initializing)
		{
			...
			provider.DoWhenFullyRecordedCharactersAvailable((fullyRecorded) =>
			{
				this.Invoke((Action) (() =>
				{
					...
					{
						_actorLabel.Text = (fullyRecorded.AllRecorded(provider.Actor) ? "\x2713  " : "") + provider.Actor;
						_characterLabel.Text = (fullyRecorded.AllRecorded(provider.Actor, provider.Character) ? "\x2713  " : "") +
						                       provider.Character;
