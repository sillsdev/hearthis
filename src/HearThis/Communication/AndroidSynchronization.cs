		...
		public static void DoAndroidSync(Project project)
		{
			...
			dlg.GotSync += (o, args) =>
			{
				...
				catch (WebException ex)
				{
					...
					{
						dlg.ProgressBox.WriteError("HearThis could not connect to the device. Are you sure you are on the same WiFi network? Could there be a firewall blocking things?");
					} else if (ex.Status == WebExceptionStatus.ConnectionClosed)
					{
						dlg.ProgressBox.WriteError("The connection to the device closed unexpectedly. Maybe you tried to do something else with it, or it went to sleep? Please try again.");
					}
					else
					{
